...
corpora_dir = "data/corpora/lid/collapsed"
...
for language_pair, data_entries in language_pairs_data.items():
    ...
    lan1 = language_pair.split("-")[0]
    upsample_ratio[lan1] = max_entries / len(data_entries)
...
temp_file_path = "data/corpora/lid/collapsed/upsampled_train.jsonl"
...
train_datasets = load_dataset(
    ...
    cache_dir="data/cache/upsampled_binary_lid",
)
...
run = wandb.init(project="L101 Coursework", entity="igorsterner")

model_name = model_checkpoint.split("/")[-1]

experiment_name = "AnE-LID"
            ...
            for file_name, eval_dataset in self.eval_datasets.items():
                ...
                for metric in metrics.keys():
                    if isinstance(metrics[metric], dict):
                        wandb.log(
                            {
                                f"{file_name}/{metric}/precision": metrics[metric]["P"],
                                f"{file_name}/{metric}/recall": metrics[metric]["R"],
                                f"{file_name}/{metric}/f1": metrics[metric]["F1"],
